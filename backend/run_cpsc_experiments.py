"""Run the 2 new CPSC graph experiments."""
...
def run(exp_id):
    ...
    for node in ordered:
        ...
        try:
            ...
            res[nid] = r
            if "error" in r:
                print(f"  {nid}: ERROR {r['error'][:120]}")
        except Exception as exc:
            res[nid] = {"error": str(exc)}
            print(f"  {nid}: EXCEPTION {str(exc)[:120]}")

    print(f"  Done in {time.time()-t0:.1f}s")

    # Show key results from Merger
    for node in ordered:
        nid = node["id"]
        ...
        if ntype == "Merger":
            ...
            if isinstance(inner, dict):
                for k, v in list(inner.items())[:10]:
                    if isinstance(v, (str, int, float, bool)):
                        print(f"  {k}: {v}")
                    elif isinstance(v, list):
                        print(f"  {k}: [n={len(v)}] {v[:3]}...")
...
if __name__ == "__main__":
    run("indus_cas_dravidian_projection")
    run("indus_cas_sign_roles")
    print("\nDone.")

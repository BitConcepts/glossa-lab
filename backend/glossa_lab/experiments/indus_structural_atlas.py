...
def run_indus_atlas(verbose: bool = True) -> dict[str, Any]:
    ...
    for n in [1, 2, 3, 4]:
        h = h_vals.get(n, {})
        _print(f"  H{n}_norm = {h.get('normalized', 0):.4f}  ({h.get('raw_nats', 0):.4f} nats)")
    ...
    _print(f"  Our corpus:      α = {cf.get('zipf_exponent', '?'):.4f} (calibrated to match Yadav)")
    ...
    for p in sorted(term_dom, key=lambda x: x.get("terminal", 0), reverse=True)[:6]:
        ...
        _print(f"    Sign {p['sign']:6}  terminal={term_pct:.1%}  freq={p.get('total', '?')}")
    ...
    _print(
        f"  Polyvalent candidates:{summary['polyvalence_candidates']} "
        f"({summary['candidate_fraction']:.0%})"
    )
    _print("\n  TOP POLYVALENT SIGNS (bimodal positional = probable dual function):")
    for c in candidates[:8]:
        _print(
            f"    Sign {c['sign']:6}  score={c['bimodality_score']:.4f}  "
    ...
    for r in ranking[:5]:
        _print(f"    {r['system'][:40]:<40}  dist={r['distance']:.4f}  ({r['writing_type']})")
    _print()
    ...
    for r in ranked[:6]:
        _print(
            f"    {r['profile']:<35}  compat={r['compatibility']:.4f}  KL={r['word_length_kl']:.4f}"
        )
    _print(f"\n  WINNER: {wsh.get('winner', '?')}")

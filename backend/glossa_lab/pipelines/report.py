...
def _make_entropy_plot(results, max_n, tmpdir):
    ...
    for name, r in results.items():
        ...
        ctype = r["corpus_type"]
        ax.plot(ns, vals, marker="o", markersize=4,
    ...
    for pub_name, pub_vals in _PUBLISHED.items():
        ...
        ax.plot(ns, pub_vals, marker="D", markersize=3,
...
def _make_zipf_plot(corpora, tmpdir):
    ...
    for name, info in corpora.items():
        ...
        ax.loglog(ranks, ranked, marker=".", markersize=2,
                  linewidth=1.2, label=f"{name} (L={len(counts)})")
...
def generate_report(corpora, output_path, max_n=6):
    ...
    author_s = ParagraphStyle("Au", parent=styles["Normal"],
    ...
    body = ParagraphStyle("B", parent=styles["Normal"],
                          spaceAfter=8, leading=14, firstLineIndent=18)
    bni = ParagraphStyle("BNI", parent=styles["Normal"],
                         spaceAfter=8, leading=14)
    cap = ParagraphStyle("Cap", parent=styles["Normal"], fontSize=8,
                         leading=10, alignment=1, spaceAfter=12,
                         spaceBefore=4, textColor=colors.HexColor("#555"))
    sm = ParagraphStyle("Sm", parent=styles["Normal"],
                        fontSize=8, leading=10)
    caveat_s = ParagraphStyle("Cav", parent=bni, fontSize=9,
    ...
    S.append(Paragraph(
        ...
        "<b>Note:</b> the Indus corpus is synthetic (see Section 9).", bni))
    ...
    S.append(Paragraph(
        ...
        "and the underlying language is unknown.", body))
    S.append(Paragraph(
    ...
    S.append(Paragraph(
        "Following Rao et al. (2009) [1] and Rao (2010) [2], we compute "
        "block entropy:", bni))
    S.append(Paragraph(
        "H<sub>N</sub> = \u2212\u03a3 p<sub>i</sub><sup>(N)</sup> "
        "ln(p<sub>i</sub><sup>(N)</sup>)",
        ParagraphStyle("Eq", parent=bni, alignment=1, fontSize=11,
                        spaceAfter=6)))
    S.append(Paragraph(
        ...
        "estimation [5] (see Limitations).", body))
    S.append(Spacer(1, 0.1 * inch))
    ...
    for i, (n, r) in enumerate(results.items(), 1):
        cd.append([str(i), n, r["corpus_type"], f"{r['symbol_count']:,}",
                   str(r["alphabet_size"]), r["description"][:52]])
    ...
    # Sample inscriptions
    S.append(Paragraph("<b>Sample Indus inscriptions</b> (Mahadevan sign "
                        "IDs):", bni))
    try:
    ...
    S.append(Paragraph(
        ...
        "corpus differences, sample sizes, and estimation methods.", bni))
    cmp = [["System", "H1 ours", "H1 Rao", "\u0394",
            "H2 ours", "H2 Rao", "\u0394"]]
    mapping = {"English": "English (Rao)", "Indus Script": "Indus (Rao)",
               "DNA": "DNA (Rao)", "Fortran": "Fortran (Rao)"}
    for on, pn in mapping.items():
        if on in results and pn in _PUBLISHED:
            ...
            cmp.append([on, f"{oh1:.3f}", f"{ph1:.3f}", f"{oh1-ph1:+.3f}",
                        f"{oh2:.3f}", f"{ph2:.3f}", f"{oh2-ph2:+.3f}"])
    S.append(_tbl(cmp, [.9*inch]+[.68*inch]*6))
    S.append(Paragraph(
        "<b>Table 3.</b> Our values vs Rao (2010). "
        "\u0394 = ours \u2212 published.", cap))

    # ═══════════ 6. ORDERING ═══════════
    S.append(Paragraph("6. Entropy Ordering and Sub-linear Growth", h1))
    srt = sorted(results.items(),
                 key=lambda x: x[1]["block_entropies"][0]["normalized"],
                 reverse=True)
    ordering = " &gt; ".join(
        f"{n} ({r['block_entropies'][0]['normalized']:.3f})"
        for n, r in srt)
    S.append(Paragraph(f"<b>H<sub>1</sub> ordering:</b><br/><br/>"
                        f"{ordering}", bni))
    ...
    for n, r in results.items():
        ...
        rd.append([n, r["corpus_type"], f"{h1v:.4f}", f"{h2v:.4f}",
                   f"{ratio:.3f}", "Yes" if ratio < 1.95 else "No"])
    S.append(_tbl(rd, [.9*inch, .7*inch, .6*inch, .6*inch, .6*inch,
                       .7*inch]))
    ...
    S.append(Paragraph(
        ...
        "distributions on log-log axes.", bni))
    S.append(Image(zipf_plot, width=6.0*inch, height=3.8*inch))
    S.append(Paragraph(
        "<b>Figure 2.</b> Zipf rank-frequency on log-log axes. "
        "L = alphabet size.", cap))
    ...
    for n, cf in cfreqs.items():
        if corpora[n].get("corpus_type") == "synthetic":
            continue
        top3 = ", ".join(f"{e['symbol']}({e['count']})"
                         for e in cf["rank_frequency"][:3])
        zd.append([n, str(cf["total_symbols"]), str(cf["unique_symbols"]),
                   f"{cf['zipf_exponent']:.2f}" if cf["zipf_exponent"]
                   else "\u2014", top3[:45]])
    S.append(_tbl(zd, [.9*inch, .6*inch, .55*inch, .55*inch, 3.5*inch]))
    S.append(Paragraph(
        "<b>Table 5.</b> Zipf analysis. \u03b1 = estimated Zipf exponent. "
        "Natural languages typically have \u03b1 \u2248 1.0.", cap))
    ...
    S.append(Paragraph(
        ...
        "visible in both Figure 1 and the ordering in Section 6.", body))
    S.append(Paragraph(
    ...
    S.append(Paragraph(
        ...
        "the linguistic hypothesis but is not proof.", body))
    S.append(Spacer(1, 0.1 * inch))
    ...
    for ref in [
        ...
        "[5] Nemenman et al. (2002). Entropy and inference, revisited. "
        "<i>NIPS</i> 14.",
        ...
        "[9] Parpola (2005). Study of the Indus script. <i>Trans. 50th "
        "ICES</i>.",
    ]:

...
def _load_fuls_format(filepath: str) -> list[list[str]]:
    ...
    return [e["inscription"]["sign_ids"] for e in entries if "inscription" in e]
...
def run_full_analysis(
    ...
    _print(
        f"§1  Corpus: N={len(flat)}  V={len(freq)}  "
    ...
    try:
        ...
        _print(
            f"H1_norm={h1.get('normalized', '?'):.3f}  "
    ...
    try:
        ...
        n_initial_dominant = sum(
            1 for p in pos.get("profiles", []) if p.get("dominant_position") == "initial"
        )
        n_terminal_dominant = sum(
            1 for p in pos.get("profiles", []) if p.get("dominant_position") == "terminal"
        )
        report["§4_positional"] = {
    ...
    try:
        ...
        _print(
            f"clusters={len(sc.get('clusters', []))}  "
    ...
    try:
        ...
        _print(
            f"candidates={summary.get('polyvalence_candidates', 0)}  "
    ...
    try:
        ...
        summary_str = "  ".join(f"{k}={v:.0%}" for k, v in sf.get("system_summary", {}).items())
    ...
    try:
        ...
        report["§13_ventris"] = {
            ...
            "n_determinatives": sum(1 for i in sign_class.values() if i["type"] == "determinative"),
        ...
        _print(
            f"syllabograms={len(syllabograms)}  "
...
def _generate_conclusions(report: dict[str, Any], system_name: str) -> list[str]:
    ...
    if N < 1000:
        conclusions.append(f"Small corpus (N={N}): statistical signals are weak. More data needed.")
    elif vn < 0.05 and hapax < 0.10:
        conclusions.append(
            f"Low V/N ({vn:.3f}) + low hapax ({hapax:.0%}) → abjad/alphabet profile."
        )
    elif vn < 0.12:
        conclusions.append(f"Moderate V/N ({vn:.3f}) → consistent with a syllabary system.")
    else:
        conclusions.append(
    ...
    if summary:
        ...
        if phonetic_n > 0:
            conclusions.append(f"Estimated phonetic sign inventory: ~{phonetic_n} signs.")
    ...
    if winner and winner != "?":
        ...
        conclusions.append(f"Word-length typology best matches: {winner} (KL={kl:.4f}).")
...
""",
    ...
    parser.add_argument(
        "--demo",
        choices=["ugaritic", "linear_b", "indus", "hebrew"],
